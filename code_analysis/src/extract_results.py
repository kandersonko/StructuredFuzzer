...
def main():
    ...
    for folder in folders:
        ...
        # get the program name from the match of the path where there is `/<name>/runs/`
        program = re.search(r'/([^/]+)/runs/', str(folder)).group(1)
    ...
    for file in files:
        ...
        print(f"Processing {program} run {run} mode {mode}")
        # Create the fuzzer log file filename with the run number and program name
        logfile = output_path / f"{program}-mode_{mode}-run_{run}-fuzzer_log"
        # Copy the fuzzer log file to the output folder
        os.system(f"cp {file} {logfile}")
